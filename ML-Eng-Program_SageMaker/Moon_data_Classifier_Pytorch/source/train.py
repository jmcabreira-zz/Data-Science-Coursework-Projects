from __future__ import print_function # future proof - print becomes a function in python 2
import argparse
import sys
import os
import json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dim',type = int, default = 2, metavar = 'IN',
                       help = 'number of input feature to model (degault = 2)')
    parser.add_argument('--hidden_dim', type = int , default = 10, metavar = 'H',
                       help = 'number of the hidden dimensions of the model(default = 10)')
    parser.add_argument('--output_dim', type = int, metavar = 'OUT',
                       help = 'number of the output dimension of the model (default = 1)')
    ...
    optimizer = optim.Adam(model.parameters(), lr = args.lr)   # Implements Adam algorithm.
    
   
    criterion = nn.BCELoss() # BCELoss as the loss function creates a criterion that measures the Binary Cross Entropy between the target and the output
